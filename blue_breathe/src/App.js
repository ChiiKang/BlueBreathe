...
const MapController = ({ routes, selectedRoute }) => {
  ...
  useEffect(() => {
    if (selectedRoute && selectedRoute.points && selectedRoute.points.length >= 2) {
      ...
      const bounds = L.latLngBounds(selectedRoute.points.map(point => point.position));
  ...
  const fetchRoutes = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/routes?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}`
      );
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-gray-100">
      <div className="bg-blue-600 text-white p-4">
        <h1 className="text-xl font-bold">Asthma-Safe Route Planner</h1>
        <p className="text-sm">Plan travel routes with the lowest pollution exposure</p>
      </div>
          ...
          <div className="mb-4">
            ...
            <div className="mb-2">
              ...
              {customDestination && (
                <div className="text-xs text-gray-500 mt-1">
                  <p>Try one of these: </p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {sampleLocations.slice(0, 4).filter(loc => loc !== origin).map(loc => (
                      <button
                        key={loc}
                        className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs"
            ...
            <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded-md">
              <p className="text-xs text-blue-700">
                This demo uses OpenStreetMap for routing and simulated air quality data.
                In a production app, it would connect to real-time air quality APIs.
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {routes.map(route => (
                  <div 
                    key={route.id}
                    className={`p-3 border rounded-md cursor-pointer ${selectedRoute?.id === route.id ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          route.avgAQI <= 50 ? 'bg-green-100 text-green-800' :
                          route.avgAQI <= 100 ? 'bg-yellow-100 text-yellow-800' :
                          route.avgAQI <= 150 ? 'bg-orange-100 text-orange-800' :
          ...
          {!origin || !destination || routes.length === 0 ? (
            <div className="h-full flex items-center justify-center text-gray-500">
              <div className="text-center p-8">
                ...
                <p className="mt-1 text-sm">Select an origin and destination to view routes.</p>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow-md p-4 h-full flex flex-col">
              ...
              <div className="w-full h-64 mb-4 border border-gray-200 rounded-lg overflow-hidden">
                {selectedRoute && selectedRoute.points && selectedRoute.points.length >= 2 && (
                  <MapContainer 
                    center={[4.2105, 101.9758]} // Center of Malaysia as default
                    zoom={6} 
                    style={{ height: '100%', width: '100%' }}
                    key={selectedRoute?.id || 'no-route'} // Force re-render on route change
                  >
                    <TileLayer
                    ...
                    <Marker position={selectedRoute.points[selectedRoute.points.length - 1].position}>
                      <Popup>
                        <b>Destination:</b> {selectedRoute.points[selectedRoute.points.length - 1].name}
                      </Popup>
                    </Marker>
                    
                    {/* Route polyline with colored segments based on AQI */}
                    {selectedRoute.points.map((point, idx) => {
                      if (idx === selectedRoute.points.length - 1) return null;
                      
                      // Create a segment between current point and next point
                      const positions = [point.position, selectedRoute.points[idx + 1].position];
                      
                      // Color based on air quality
                      const color = 
                        point.aqi <= 50 ? '#22c55e' :  // green-500
                        point.aqi <= 100 ? '#eab308' : // yellow-500
                        point.aqi <= 150 ? '#f97316' : // orange-500
                        '#ef4444';                      // red-500
                      ...
                      return (
                        ...
                        >
                          <Popup>
                            <div>
                              <b>Air Quality:</b> {point.aqi} AQI
                              <br />
                              <span style={{color: color}}>●</span> {getAQIStatus(point.aqi)}
                            </div>
                          </Popup>
                        </Polyline>
                      );
                  ...
                  <div className="relative pt-4">
                    <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-gray-200">
                      {selectedRoute.points.map((point, index) => {
                        // Skip the first point as it's the origin
                        if (index === 0) return null;
                        
                        // Calculate segment width based on position in route
                        const segmentWidth = 100 / (selectedRoute.points.length - 1);
                        
                        // Get color based on AQI
                        const bgColorClass = 
                          point.aqi <= 50 ? 'bg-green-500' :
                          point.aqi <= 100 ? 'bg-yellow-500' :
                          point.aqi <= 150 ? 'bg-orange-500' :
                          'bg-red-500';
                    ...
                    <div className="flex justify-between">
                      {selectedRoute.points.map((point, index) => (
                        <div key={index} className="flex flex-col items-center">
                          ...
                          <span className="mt-1 text-xs text-gray-500">{point.aqi} AQI</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="mt-4">
                ...
                <div className={`p-3 rounded-md ${
                  selectedRoute?.avgAQI <= 50 ? 'bg-green-50 border border-green-200' :
                  selectedRoute?.avgAQI <= 100 ? 'bg-yellow-50 border border-yellow-200' :
                  selectedRoute?.avgAQI <= 150 ? 'bg-orange-50 border border-orange-200' :
                  'bg-red-50 border border-red-200'
                }`}>
                  <div className="font-medium mb-1">
                    {selectedRoute?.avgAQI <= 50 ? 'Low Risk' :
                     selectedRoute?.avgAQI <= 100 ? 'Moderate Risk' :
                     selectedRoute?.avgAQI <= 150 ? 'High Risk - Consider Alternatives' :
